...
void Canvas::SetFresh(bool b)
{
    if (b) Fl::add_timeout(0.5, Canvas::FreshThread, this);
    else Fl::remove_timeout(Canvas::FreshThread, this);
}
...
{
    float aspect = (w()+1.0f)/(h()+1.0f);
    if (_persp) gluPerspective(27.5, aspect, 0.1, 10.0);
    else glOrtho(-1.0*aspect, 1.0*aspect, -1.0, 1.0, 0.1, 10.0);
...
int Canvas::handle(int event)
{
    ...
    case FL_PUSH:
        if (Fl::event_button() == FL_RIGHT_MOUSE) {
            EditObject(Fl::event_x(), Fl::event_y());
            return 0;
        }
        if (Fl::event_button() == FL_MIDDLE_MOUSE && _select)
